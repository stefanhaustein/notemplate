...
element.registerHelpers = function(context=window, protectAgainstContextOverwrite=true) {
  ...
  for (const name of elementNames) {
    if (name in context && protectAgainstContextOverwrite) {
      console.warn('Will skip registering function %s on context, because it already exists', name);
    } else {
      context[name] = (...content) => element(name, ...content)
    }
  }
  return context;
}
